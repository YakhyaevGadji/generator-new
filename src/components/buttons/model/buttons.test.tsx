...
describe('Buttons', () => {
    ...
    it('click button', () => {
        ...
        const generatorBtn = screen.getByText(/Генератор/i);

        expect(btn1).toBeInTheDocument();
        expect(btn2).toBeInTheDocument();
        expect(generatorBtn).toBeInTheDocument();

        fireEvent.click(btn1);

        expect(store.dispatch).toHaveBeenCalledWith(setButtonData({ kit: 'one', count: 4 }));
        expect(store.dispatch).toHaveBeenCalledWith(resetSelectedCards());
